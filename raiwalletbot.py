...
def price_text(bot, update):
	...
	high_price = ('%.8f' % (max(float(price[0][1]), float(price[1][1]), float(price[2][1]), float(price[0][0])) / (10 ** 8)))
	low_price = ('%.8f' % (min(float(price[0][2]), float(price[1][2]), float(price[2][2])) / (10 ** 8)))
